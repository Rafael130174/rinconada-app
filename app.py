...
<head>
    ...
    <style>
        ...
        .pago-movil-box { background: #f8f9fa; border: 1px solid #cbd5e1; padding: 10px; border-radius: 6px; margin-bottom: 12px; font-size: 13px; color: #334155; }
    </style>
</head>
<body>
    ...
    <div class="card">
        ...
        <div style="background: #e8f0fe; border: 1px solid #aecbfa; padding: 12px; border-radius: 8px; margin-bottom: 15px;">
            ...
            <div class="pago-movil-box">
                <b style="color: #174ea6; display: block; margin-bottom: 4px;">📱 Datos para Pago Móvil:</b>
                • <b>Banco:</b> Banesco / BNC<br>
                • <b>Teléfono:</b> 04128424972<br>
                • <b>Cédula / RIFF:</b> 12275066
            </div>
...
def escrutar():
  ...
  for b in boletos:
    ...
    aciertos = sum(
        1 for i in range(1, 7) if resultados[str(i)] in cuadro.get(str(i), [])
    )
    
    if aciertos == 6:
      ganadores_6.append(b)
    elif aciertos == 5:
      ganadores_5.append(b)
    elif aciertos == 4:
      ganadores_4.append(b)

    if b.pote_3 == resultados['3'] and b.pote_6 == resultados['6']:
      ganadores_pote.append(b)

  ganadores_resumen = []

  def procesar_premio_lista(lista_boletos, monto_total_pozo, tipo_texto):
    if not lista_boletos:
      return
    monto_unitario = monto_total_pozo / len(lista_boletos)
    for b in lista_boletos:
      ...
      if user_cliente:
        user_cliente.balance += monto_unitario
