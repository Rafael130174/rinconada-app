...
<body>
    <div class="login-card">
        ...
        <a href="/recuperar" class="back-link" style="color: #d93025;">¿Olvidaste tu contraseña?</a>
...
<head>
    ...
    <style>
        ...
        .error { color: red; font-size: 13px; margin-bottom: 10px; }
        .captcha-box { background: #f1f3f4; padding: 10px; border-radius: 6px; margin: 10px 0; font-size: 14px; }
        .back-link { display: block; margin-top: 12px; color: #1a73e8; text-decoration: none; font-size: 13px; }
    </style>
</head>
<body>
    <div class="login-card">
        ...
        <form method="POST">
            ...
            <div class="captcha-box">
                <label style="margin-bottom: 4px; display:block;">🔒 Verificación Anti-Bot:</label>
                <b>¿Cuánto es {{ captcha_pregunta }}?</b>
                <input type="number" name="captcha_respuesta" placeholder="Respuesta" required style="margin-top: 6px;">
            </div>
...
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Recuperar Contraseña - La Rinconada Oriental</title>
    <style>
        body { font-family: sans-serif; background: #1a73e8; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; }
        .login-card { background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); width: 90%; max-width: 360px; text-align: center; }
        h2 { color: #d93025; margin-bottom: 15px; font-size: 20px; }
        input { width: 100%; padding: 12px; margin: 8px 0; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; font-size: 15px; }
        button { background: #d93025; color: white; border: none; padding: 12px; width: 100%; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; margin-top: 10px; }
        .error { color: red; font-size: 13px; margin-bottom: 10px; }
...
<body>
    ...
    <div class="card" style="border: 2px solid #28a745;">
        <h3 style="color: #28a745;">🎟️ Tickets Activos de la Jornada ({{ boletos_activos|length }})</h3>
        {% if boletos_activos %}
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr>
                    <td><b>{{ b.codigo_ticket }}</b></td>
                    <td>{{ b.cliente }}<br><small>{{ b.telefono }}</small></td>
                    <td>{{ b.loteria }}</td>
                    <td style="color: #28a745; font-weight: bold;">{{ "%.2f"|format(b.monto) }} Bs.</td>
                </tr>
                {% endfor %}
            </tbody>
        </table>
        {% else %}
        <p style="text-align: center; color: #777; font-size: 13px;">No hay tickets registrados para esta jugada aún.</p>
        {% endif %}
    </div>
...
def registro():
  if request.method == 'POST':
    ...
    # Validar CAPTCHA
    try:
      resp_user = int(request.form.get('captcha_respuesta', '0'))
    except ValueError:
      resp_user = -1
      
    if resp_user != session.get('captcha_res', -999):
      pregunta, respuesta = generar_captcha()
      session['captcha_res'] = respuesta
      return render_template_string(
          REGISTRO_HTML, error='Verificación anti-bot incorrecta. Inténtalo de nuevo.', captcha_pregunta=pregunta
      )
    ...
    if User.query.filter_by(username=username).first():
      ...
      session['captcha_res'] = respuesta
      return render_template_string(
          REGISTRO_HTML, error='El usuario ya existe. Elige otro.', captcha_pregunta=pregunta
      )
  ...
  pregunta, respuesta = generar_captcha()
  session['captcha_res'] = respuesta
  return render_template_string(REGISTRO_HTML, captcha_pregunta=pregunta)
...
def recuperar():
  if request.method == 'POST':
    ...
    if resp_user != session.get('captcha_res', -999):
      ...
      session['captcha_res'] = respuesta
      return render_template_string(
          RECUPERAR_HTML, error='Verificación anti-bot incorrecta.', captcha_pregunta=pregunta
      )
    ...
    if not user:
      ...
      session['captcha_res'] = respuesta
      return render_template_string(
          RECUPERAR_HTML, error='El usuario no existe.', captcha_pregunta=pregunta
      )

    user.password_hash = generate_password_hash(nueva_password)
    db.session.commit()
    return render_template_string(RECUPERAR_HTML, success='¡Contraseña actualizada con éxito! Ya puedes iniciar sesión.', captcha_pregunta="1 + 1")

  pregunta, respuesta = generar_captcha()
  session['captcha_res'] = respuesta
  return render_template_string(RECUPERAR_HTML, captcha_pregunta=pregunta)
...
def home():
  ...
  recargas_pendientes = (
      RechargeRequest.query.filter(RechargeRequest.status.ilike('%pendiente%'))
  ...
  boletos_activos = Boleto.query.all()
  return render_template_string(
      MAIN_HTML,
...
def aprobar_recarga(id):
  ...
  if recarga and 'pendiente' in recarga.status.lower():
