...
class User(UserMixin, db.Model):
  ...
  )  # Billetera / Saldo disponible del usuario
...
<body>
    ...
    <div class="card">
        ...
        <!-- SECCIÓN DE RECARGA DE SALDO (MÍNIMO 1000 BS) -->
        <div style="background: #e8f0fe; border: 1px solid #aecbfa; padding: 12px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="color: #174ea6; margin-top:0;">💳 Recargar Billetera (Mínimo 1,000 Bs.)</h3>
            <form method="POST" action="/solicitar_recarga">
                <label>Monto a Recargar (Bs.):</label>
                <input type="number" step="0.01" name="amount" placeholder="Mínimo 1000 Bs" required>
                
                <label>Banco Emisor:</label>
                <input type="text" name="bank" placeholder="Ej: Banco de Venezuela" required>
                
                <label>Número de Referencia:</label>
                <input type="text" name="reference" placeholder="Últimos dígitos o referencia" required>
    ...
    <!-- PANEL DE ADMINISTRACIÓN DE RECARGAS PENDIENTES -->
    <div class="card" style="border: 2px solid #174ea6;">
        ...
        {% else %}
        <p style="text-align: center; color: #777; font-size: 13px;">No hay recargas pendientes por aprobar.</p>
        {% endif %}
    </div>
    ...
    <div class="card" style="border: 2px solid #1a73e8;">
        ...
        <h4 style="margin-top: 20px;">📋 Lista de Usuarios Registrados y Comisiones (10%)</h4>
        <table>
            <thead>
                <tr>
                    <th>Usuario</th>
...
def solicitar_recarga():
  ...
  if amount < 1000:
    return redirect(url_for('home'))  # Límite mínimo de 1,000 Bs
...
def vender():
  ...
  if error or not pote_3 or not pote_6:
    ...
    ticket_res = (
        '⚠️ Error: Saldo insuficiente en tu billetera. Por favor recarga más de'
        ' 1,000 Bs.'
    )
  else:
    ...
    # Calcular 10% de comisión si interviene un vendedor o si el usuario actual es vendedor
    comision_vendedor = monto_total * 0.10
    vendedor_a_acreditar = None
    if current_user.role == 'vendedor':
      current_user.commission_earned += comision_vendedor
      vendedor_a_acreditar = current_user.id
    else:
      # Si compra online, opcionalmente se puede dejar asignado al admin o sin vendedor directo
      vendedor_a_acreditar = current_user.id
...
💵 TOTAL PAGADO (Descontado): {monto_total:.2f} Bs.
----------------------------------------
✅ Ticket Registrado y Pagado."""
...
def escrutar():
  ...
  for b in boletos:
    ...
    aciertos = sum(
        1 for i in range(1, 7) if resultados[str(i)] in cuadro[str(i)]
    )
    ...
    if aciertos == 6:
      ...
      ganadores.append({
          ...
          'loteria': b.loteria,
          'vendedor': vendedor_nombre,
          'tipo': '🥈 Segundo Premio (5 Aciertos)',
      })
    elif aciertos == 4:
      cant_4 += 1
      ganadores.append({
