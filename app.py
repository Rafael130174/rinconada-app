...
# 1. INSTANCIACIÓN DE LA APLICACIÓN (DEBE IR PRIMERO QUE TODO)
app = Flask(__name__)
...
class User(UserMixin, db.Model):
    ...
    role = db.Column(db.String(20), default='cliente') # 'admin', 'vendedor', 'cliente'
...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="password"], select { width: 100%; padding: 10px; margin-top: 4px; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; font-size: 15px; }
...
<body>
    ...
    <div class="user-bar">
        {% if current_user.is_authenticated %}
            <span>👤 <b>{{ current_user.username }}</b> ({{ current_user.role.upper() }})</span>
    ...
    <div class="card">
        ...
        <!-- FORMULARIO HABILITADO TANTO PARA VENDEDORES, ADMIN COMO PARA CLIENTES AUTÓNOMOS -->
        {% if current_user.is_authenticated %}
    ...
    <!-- PANEL DE ADMINISTRACIÓN DE USUARIOS (SOLO ADMIN CREA VENDEDORES) -->
    {% if current_user.is_authenticated and current_user.role == 'admin' %}
    <div class="card" style="border: 2px solid #1a73e8;">
        ...
        <h4 style="margin-top: 20px;">📋 Lista de Usuarios Registrados</h4>
        <table>
            <thead>
                <tr>
                    <th>Usuario</th>
    ...
    <div class="card" style="border: 2px solid #28a745;">
        ...
        <div class="reporte-box">
            <h4 style="margin: 0 0 10px 0; text-align:center;">📊 Balance General (Admin)</h4>
        ...
        {% if ganadores %}
            ...
            <div class="winner">
                ...
                <br>• Registrado por: <i>{{ g.vendedor }}</i>
                {% endif %}
            </div>
            {% endfor %}
        {% else %}
...
def registro():
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            return render_template_string(REGISTRO_HTML, error="El usuario ya existe. Elige otro.")
        user = User(
            username=username,
            password_hash=generate_password_hash(password),
            role='cliente' # El público se registra autónomamente como cliente
        )
        db.session.add(user)
        db.session.commit()
        login_user(user)
...
def home():
    ...
    return render_template_string(MAIN_HTML, animalitos=ANIMALITOS, validas=validas_list, acum=acum, historial=historial, usuarios_lista=usuarios_lista, ganadores=None, financiero=None)
...
def crear_usuario():
    ...
    user = User(
        ...
        role='vendedor' # Solo el admin crea vendedores
    )
    db.session.add(user)
    db.session.commit()
    return redirect(url_for('home'))
...
@login_required
def vender():
    # Permitimos que Admin, Vendedores y Clientes generen jugadas/tickets
...
✅ Ticket Registrado."""
    ...
    return render_template_string(MAIN_HTML, animalitos=ANIMALITOS, validas=validas_list, ticket=ticket_res, acum=acum, historial=historial, usuarios_lista=usuarios_lista, ganadores=None, financiero=None)
...
def escrutar():
    ...
    for b in boletos:
        ...
        if aciertos == 6:
            cant_6 += 1
            ganadores.append({"cliente": b.cliente, "telefono": b.telefono, "loteria": b.loteria, "vendedor": vendedor_nombre, "tipo": "🥇 Primer Premio (6 Aciertos)"})
        elif aciertos == 5:
            cant_5 += 1
            ganadores.append({"cliente": b.cliente, "telefono": b.telefono, "loteria": b.loteria, "vendedor": vendedor_nombre, "tipo": "🥈 Segundo Premio (5 Aciertos)"})
        elif aciertos == 4:
            cant_4 += 1
            ganadores.append({"cliente": b.cliente, "telefono": b.telefono, "loteria": b.loteria, "vendedor": vendedor_nombre, "tipo": "🥉 Tercer Premio (4 Aciertos)"})
            
        if b.pote_3 == resultados['3'] and b.pote_6 == resultados['6']:
            cant_pote += 1
            ganadores.append({"cliente": b.cliente, "telefono": b.telefono, "loteria": b.loteria, "vendedor": vendedor_nombre, "tipo": "🔥 Pote Fijo"})
    ...
    historial = Historial.query.order_by(Historial.id.desc()).all()
    usuarios_lista = User.query.all()
    return render_template_string(MAIN_HTML, animalitos=ANIMALITOS, validas=validas_list, financiero=financiero, ganadores=ganadores, acum=acum_anterior, historial=historial, usuarios_lista=usuarios_lista)
