...
def solicitar_recarga():
  ...
  # Actualizado a 1000 Bs como tope máximo permitido
  if amount > 1000:
    return redirect(url_for('home'))
...
def vender():
  ...
  # Hora ajustada para la zona horaria de Venezuela (UTC-4)
  ...
  if error or not pote_3 or not pote_6:
    ...
    ticket_res = (
        '⚠️ Error: Saldo insuficiente en tu billetera. Por favor recarga más'
        ' saldo.'
    )
  else:
...
def escrutar():
  ...
  # Ajuste de hora también para el registro diario (opcional pero recomendado)
